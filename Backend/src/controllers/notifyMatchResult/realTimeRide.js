...
const sendNotificationAndMessageToUsers = (req, res) => {
    ...
    req.users.forEach( (user) => {
        ...
        if(userId === driverId) {
            text += `your passenger is [${rider.nickname}], contact: +91${rider.contact}. Travel distance is ${displayDistance} m and estimated duration is ${displayDuration} mins`;
        }else{
            text += `your driver is [${driver.nickname}], contact: +91${driver.contact}, carplate: ${driver.carplate}. Travel distance is ${displayDistance} m and estimated duration is ${displayDuration} mins`;
        }
